...
usage()
{
    echo "Usage: $PROGRAM -b basename [-g GATK_path] [-h help] [-i readgroup_id] -r reference.fasta -s sample.fastq"
}
...
do
    case $opt in
        ...
        r) refseq=$OPTARG
            ...
            ;;
        s) sampseq=$OPTARG
            sampseq_ext=$(echo $sampseq |awk -F . '{if (NF>1) {print $NF}}')
            ;;
...
if ! [ -f "$sampseq" ]
then
    echo "sample fasta files required with option -s"
    exit 1
fi

if ! [ "X$sampseq_ext" = "Xfq" ] && ! [ "X$sampseq_ext" = "Xfastq" ]
then
    echo "sample files must have fq or fastq extension"
    exit 1
fi

echo "now aligning $sampseq"
bwa aln $refseq \
    $sampseq > $sampseq.sai
